                ...
                <div className="flex flex-col bg-black-background">
                    <div className="flex flex-col justify-between items-center">
                        ...
                        {chunkWorks(AllWorks, 3).map((chunk, index) => (
                            <div key={index}
                                 className="flex flex-col tablet:flex-row justify-center items-center space-x-10">
                                {chunk.map((work) => (
                                    <div key={work.title}>
                                        <Typography variant="h4" color="neutral-1" bold={true}>
                                            {work.title}
                                        </Typography>
                                        <AllWorksImage urlImage={work.urlImage} text={work.category} linkName={work.linkName}/>
                                        <Padding/>
                                    </div>
                                ))}
                            </div>
